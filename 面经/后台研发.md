...
1. HashMap不是线程安全的
2. HashTable是线程安全的一个Collection（hashtable的put和get方法都使用了同步函数，保证线程安全）
3. HashMap是HashTable的轻量级实现（非线程安全实现），HashMap允许空（null）键值（null）
...
- 校验和
  ...
  在数据传输的过程中，将发送的数据段都<u>当做一个16位的整数</u>。将这些整数加起来。并且前面的进位不能丢弃，补在后面，最后取反，得到校验和。
